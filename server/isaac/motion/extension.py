...
async def f_step(articulation, controller, provider, gamepad, se3, joint, link, step):
    ...
    command[0][5] = 0.0
    carb.log_error(f"[motion.extension] [run_call] clamp Command: {command}, {gripper}")
...
def f_data(
    ...
    return #debug don't print messy data
    ...
    carb.log_info(f"[motion.extension] [run_call] Annotator callback - camera: {entries}")
    ...
    carb.log_info(f"[motion.extension] [run_call] Articulation callback - state: {state}")
...
async def run_tick(
    ...
    while True:
        ...
        try:
            ...
            carb.log_info(f"[motion.extension] [run_call] [run_tick] Channel callback done")
            step = await interface.tick(data)
            carb.log_info(f"[motion.extension] [run_call] [run_tick] Interface step {step}")
            f_step(
                articulation=articulation,
                controller=controller,
                provider=provider,
...
async def run_call(session, call):
    ...
    carb.log_info(f"[motion.extension] [run_call] Articulation Controller: {controller}")

    carb.log_info(f"[motion.extension] [run_call] Articulation Joint: {articulation.dof_paths}")
    joint = list(
        e
    ...
    carb.log_info(f"[motion.extension] [run_call] Articulation Link: {articulation.link_paths}")
